var i=()=>{Shiny.addCustomMessageHandler("bind-network-keyboard-shortcuts",t=>{let d=HTMLWidgets.find(t.id).network;$(document).on("keydown",e=>{e.shiftKey&&e.keyCode==69&&(d.addEdgeMode(),$("button.vis-back").hide(),$(".vis-separator-line").hide())}),$(document).on("keydown",e=>{e.key==="Escape"&&(d.disableEditMode(),d.enableEditMode(),$(".vis-close").hide(),$(".vis-edit").hide(),$(".vis-separator-line").hide())})})};var a=()=>{Shiny.addCustomMessageHandler("hide-vis-dropdown",t=>{$(t).css("visibility","hidden")}),Shiny.addCustomMessageHandler("toggle-manipulation-ui",t=>{$(".vis-close").hide(),$(".vis-edit").hide(),$(".vis-separator-line").hide(),t.value?$(".vis-manipulation").show():$(".vis-manipulation").hide()}),Shiny.addCustomMessageHandler("hide-sidebars-toggles",t=>{$(`button[aria-controls="${t.ns}-dashboard"]`).css("visibility","hidden"),$(`button[aria-controls="${t.ns}-properties"]`).css("visibility","hidden")})};var b=t=>{var d=HTMLWidgets.find("#"+t),e;return typeof d<"u"&&(e=d.getWidget()),e},o=()=>{Shiny.addCustomMessageHandler("add-grid-widget",t=>{var d=b(t.id);typeof d<"u"&&(t.data.content=`<div
              id="grid-stack-item-content-${t.data.id}"
              class="bg-white p-2 border rounded-4">
            </div>`,d.addWidget(t.data),$(`#grid-stack-item-content-${t.data.id}`).append($(`#${t.data.id}`)),$(`#${t.data.id}`).is(":hidden")&&$(`#${t.data.id}`).show())}),Shiny.addCustomMessageHandler("move-widget-to-sidebar",t=>{$(t.id).append($(t.block_id))}),Shiny.addCustomMessageHandler("update-grid-zoom",t=>{$(t.id).css("zoom",t.zoom)})};var r=(t,d)=>{let e={x:d.clientX,y:d.clientY};Shiny.setInputValue(t,e,{priority:"event"})},s=()=>{Shiny.addCustomMessageHandler("capture-mouse-position",t=>{document.addEventListener("contextmenu",d=>{r(t,d)})}),$("body").click(t=>{$(t.target).closest(".node-menu-card").length===0&&$(".node-menu-card").hide()}),Shiny.addCustomMessageHandler("show-node-menu",t=>{if($(`#${t.id}`).length===0){let n=t.id.split(t.ns)[1],c=`<div class="card node-menu-card" style="width: 18rem; position: absolute;" id="${t.id}">
